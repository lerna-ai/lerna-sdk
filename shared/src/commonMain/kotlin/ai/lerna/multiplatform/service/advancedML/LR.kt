...
{
    val embeddingTable = LernaEmbeddingsMap<Int>(num_features = totalNumFeatures, size=embeddingSize)
    ...
    val finalLayerParams = StackedLayersParameters(
        LayerInterface(size = numEmbeddings * embeddingSize + sensorEmbeddingDim , type = LayerType.Input.Dense),
        LayerInterface(size = 1, activationFunction = Sigmoid, connectionType = LayerType.Connection.Feedforward))
    ...
    override fun forward(input: SimpleExample): DenseNDArray{
        val embedding = embeddingProcessor.forward(input.categFeatures, input.numericalFeatures, input.multiHotFeatures).map { it as DenseNDArray }

        val concatInput: D2Array<Float> = if (embedding.isNotEmpty())
        {
            val flatArray = mk.ndarray(mk[embedding.map { it.storage.toList()}]).reshape(numEmbeddings * embeddingSize, 1)
            // If no sensor data is provided, then the input is the attention output
            if (input.sensors == null) {
                flatArray
            }
            // If sensor data is provided, then pass it through the sensor layer and concatenate it with the flattened embedding output
            else
            {
    ...
    override fun backward(outputErrors: DenseNDArray){
        ...
        // InputErrors is the gradient of the MLP layer with respect to its input (aka the output of the embedding layer)
        // Its dimension is of the size: numEmbeddings * embeddingSize + sensorEmbeddingDim.
        val concatErrors = finalLayer.getInputErrors(copy=true)


        if (numEmbeddings > 0) {
            // separate the gradients of dim (numEmbeddings * embeddingSize) for backpropagation into the embedding layer
            val embeddingErrors: List<DenseNDArray> = concatErrors.storage[0 until numEmbeddings * embeddingSize].reshape(numEmbeddings, embeddingSize).
    ...
    fun getWeights() : AdvancedMLItem {
        ...
        weights.embedding= embeddingTable.getParams()
    ...
    fun setWeights(modelWeights: AdvancedMLItem) {
        ...
        val sensorsParams = modelWeights.sensors
        if(sensorsParams!=null)
            sensorLayerParameters?.setParams(sensorsParams)

        val layerParams = modelWeights.lastlayer
        if(layerParams!=null)
